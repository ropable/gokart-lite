...
if PYTHON_VERSION == 3:
    profile_re = re.compile(
        b"gokartProfile\s*=\s*(?P<profile>\{.+\})\s*;?\s*exports.+default.+gokartProfile",
        re.DOTALL,
    )
else:
    profile_re = re.compile(
        "gokartProfile\s*=\s*(?P<profile>\{.+\})\s*;?\s*exports.+default.+gokartProfile",
        re.DOTALL,
    )
...
def _get_profile(app):
    ...
    if profile:
        ...
        if (
            repr(os.path.getmtime(appPath)) != profile["mtime"]
            or os.path.getsize(appPath) != profile["size"]
        ):
            profileChanged = True
            profile = None
    ...
    if not profile:
        ...
        profile = {
            ...
            "profile": demjson3.decode(profile),
        }
        m = hashlib.md5()
        m.update(file_data)
        profile["profile"]["build"]["md5"] = base64.urlsafe_b64encode(m.digest())

        # Python 2/3 compatibility:
        if PYTHON_VERSION == 3:
            profile["profile"]["build"]["md5"] = (
                profile["profile"]["build"]["md5"].rstrip(b"=").decode("utf8")
            )
        else:
            profile["profile"]["build"]["md5"] = profile["profile"]["build"][
                "md5"
            ].rstrip("=")
    ...
    if vendorProfile:
        ...
        if (
            repr(os.path.getmtime(vendorPath)) != vendorProfile["mtime"]
            or os.path.getsize(vendorPath) != vendorProfile["size"]
        ):
            profileChanged = True
            vendorProfile = None
    ...
    if not vendorProfile:
        ...
        vendorProfile = {
            ...
            "vendorMD5": base64.urlsafe_b64encode(m.digest()),
        }

        # Python 2/3 compatibility:
        if PYTHON_VERSION == 3:
            vendorProfile["vendorMD5"] = (
                vendorProfile["vendorMD5"].rstrip(b"=").decode("utf8")
            )
        else:
            vendorProfile["vendorMD5"] = vendorProfile["vendorMD5"].rstrip("=")
    ...
    envPath = os.path.join(
        BASE_DIST_PATH, "release", "static", "js", "{}-{}.env.js".format(app, ENV_TYPE)
    )
    if not os.path.exists(envPath):
        ...
        if envProfile:
            ...
            if (
                repr(os.path.getmtime(envPath)) != envProfile["mtime"]
                or os.path.getsize(envPath) != envProfile["size"]
            ):
                profileChanged = True
                envProfile = None
        ...
        if not envProfile:
            ...
            envProfile = {
                ...
                "envMD5": base64.urlsafe_b64encode(m.digest()),
            }

            # Python 2/3 compatibility:
            if PYTHON_VERSION == 3:
                envProfile["envMD5"] = envProfile["envMD5"].rstrip(b"=").decode("utf8")
            else:
                envProfile["envMD5"] = envProfile["envMD5"].rstrip("=")
    ...
    if not os.path.exists(stylePath):
        ...
        if styleProfile:
            ...
            if (
                repr(os.path.getmtime(stylePath)) != styleProfile["mtime"]
                or os.path.getsize(stylePath) != styleProfile["size"]
            ):
                profileChanged = True
                styleProfile = None
        ...
        if not styleProfile:
            ...
            styleProfile = {
                ...
                "styleMD5": base64.urlsafe_b64encode(m.digest()),
            }

            # Python 2/3 compatibility:
            if PYTHON_VERSION == 3:
                styleProfile["styleMD5"] = (
                    styleProfile["styleMD5"].rstrip(b"=").decode("utf8")
                )
            else:
                styleProfile["styleMD5"] = styleProfile["styleMD5"].rstrip("=")
...
def index(app):
    try:
        ...
        if profile["dependents"]["vendorMD5"] != profile["build"]["vendorMD5"]:
            raise Exception(
                "Application was built base on outdated vendor library, please build application again."
            )
